class ArticlesController < ApplicationController
  ...
  def index
    @articles = current_user.articles.includes(:users, :system_users).where('status = ?', Article::Status::DRAFT)
    @proxy_articles = current_user.proxy_articles.includes(:users, :system_users).where('status = ?', Article::Status::DRAFT) - @articles
